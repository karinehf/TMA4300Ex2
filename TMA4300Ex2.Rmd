...
drawKappaU = function(n, alpha_u, beta_u, u, R){
  rgamma(1, shape = (n-1)/2+alpha_u, rate = beta_u + t(u)%*%R%*%u/2)
}

drawKappaV = function(n, alpha_v, beta_v, eta, u){
  rgamma(1, shape = n/2+alpha_v, rate = beta_v+sum((eta-u)^2)/2)
}
...
MCMCOral = function(M, y, starting_values, alpha_u, beta_u, alpha_v, beta_v, E, R){
  ...
  for(i in 2:M){
    ...
    q_eta = dmvnorm.canonical(eta, kappa_v*u + b_func(eta_prev,y,E), diag.spam(as.vector(c_func(eta_prev,E))))
    q_eta_prev = dmvnorm.canonical(eta_prev, kappa_v*u + b_func(eta,y,E), diag.spam(as.vector(c_func(eta,E))))
    log_acceptance = p_eta-p_eta_prev+q_eta_prev-q_eta
    #logFullCondEta(eta, kappa_v, u, y, E)-logFullCondEta(eta_prev, kappa_v, u, y, E) + dmvnorm.canonical(eta_prev, kappa_v*u + b_func(eta,y,E), diag.spam(as.vector(c_func(eta,E))))-dmvnorm.canonical(eta, kappa_v*u + b_func(eta_prev,y,E), diag.spam(as.vector(c_func(eta_prev,E))))
...
INLAOral = inla(
  formula = Y ~ f(Region.struct, model = "besag", graph = g, hyper = list(prec=list(prior="loggamma", param=c(alpha_u,beta_u))), constr = FALSE) + 
    f(Region, model = "iid", hyper = list(prec=list(prior="loggamma", param=c(alpha_v,beta_v))), constr = TRUE)-1, 
 family = "poisson",
 E=E,
 data = OralData
)
...
INLAOSmoking1 = inla(
  formula = Y ~ f(Region.struct, model = "besag", graph = g, hyper = list(prec=list(prior="loggamma", param=c(alpha_u,beta_u))), constr = FALSE) + 
    f(Region, model = "iid", hyper = list(prec=list(prior="loggamma", param=c(alpha_v,beta_v))), constr = TRUE)-1 + smoking, 
 family = "poisson",
 E=E,
 data = OralData
)
...
INLASmoking2 = inla(
  formula = Y ~ f(Region.struct, model = "besag", graph = g, hyper = list(prec=list(prior="loggamma", param=c(alpha_u,beta_u))), constr = FALSE) + 
    f(Region, model = "iid", hyper = list(prec=list(prior="loggamma", param=c(alpha_v,beta_v))), constr = TRUE) + f(smoking, model = 'rw2', hyper = list(prec = list(prior = "loggamma")), constr = FALSE)-1, 
 family = "poisson",
 E=E,
 data = OralData
)
