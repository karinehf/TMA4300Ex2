...
```{r, fig:germany_MCMC, fig.width=5, fig.height=4, fig.cap="\\label{fig:germany_MCMC} Posterior spatial dependency (exp(u)) of cases of oral cancer accross regions in Germany."}
post_median = exp(apply(samples$u, 1, median))

germany.plot(post_median, col=col, legend=TRUE)
```

We see that the general tendencies from figure \ref{fig:germany_y} in the beginning of the report are also present in figure \ref{fig:germany_MCMC}, but that the values are a lot smoother accross regions. Figure \ref{fig:germany_MCMC} shows exp(median) of the posterior distribution for spatial correlation in number of oral cancer cases. We do not have any explanatory variables in our model. This means that the figure shows what would be the expected number of cancer cases accross regions (divided by the expected number of cases for demographic reasons, $E_i$) if there were no noise present. 

#Exercise 6: INLA

##a)
R-INLA can also be used to estimate oral cancer across regions in Germany. First, we 
```{r}
...
INLASmoking1 = inla(
  formula = Y ~ 
    f(Region.struct, model = "besag", graph = g, 
      hyper = list(prec=list(prior="loggamma", param=c(alpha_u,beta_u))), constr = FALSE) + 
    f(Region, model = "iid", 
