...
drawKappaU = function(n, alpha_u, beta_u, u, R){
  rgamma(1, (n-1)/2+alpha_u, rate = beta_u + t(u)%*%R%*%u/2)
}
...
logFullCondEta = function(eta, kappa_v, u, y, E){
  -kappa_v*sum(eta^2)/2+sum(eta*u)*kappa_v + sum(eta*y)-sum(exp(eta)*E)
}
...
MCMCOral = function(M, y, starting_values, alpha_u, beta_u, alpha_v, beta_v, E, R){
  ...
  for(i in 2:M){
    ...
    log_acceptance = logFullCondEta(eta, kappa_v, u, y, E)-logFullCondEta(eta_prev, kappa_v, u, y, E) + dmvnorm.canonical(eta_prev, kappa_v*u + b_func(eta,y,E), diag.spam(as.vector(c_func(eta,E))))-dmvnorm.canonical(eta, kappa_v*u + b_func(eta_prev,y,E), diag.spam(as.vector(c_func(eta_prev,E))))
...
#Trace plots Kappa
trace.kappa_u = 
  ggplot(data.frame(kappa = samples$kappa_u[500:M]), aes(x=500:M, y = kappa)) + geom_line()+ ggtitle("MCMC trace plots: Kappa") + theme(plot.title = element_text(hjust = 0.5)) + xlab('Iterations') + ylab('kappa_u')

trace.kappa_v = 
  ggplot(data.frame(kappa = samples$kappa_v[500:M]), aes(x=500:M, y = kappa)) + geom_line() + xlab('Iterations') + ylab('kappa_v')
...
INLAOral = inla(
  formula = Y ~ f(Region.struct, model = "besag", graph = g, hyper = list(prec=list(prior="loggamma", param=c(alpha_u,beta_u))), constr = FALSE) + 
    f(Region, model = "iid", hyper = list(prec=list(prior="loggamma", param=c(alpha_v,beta_v))), constr = FALSE)-1, 
 family = "poisson",
 E=E,
 data = OralData
)
...
u_inla = INLASmoking2$marginals.random[[1]]

plot(u_inla[[1]], type = 'l')

#inla.hyperpar(INLAOral)
```
```{r}
u_median = c()

for (i in 1:544){
  u_median[i] = median(u_inla[[i]])
}
...
INLAOSmoking1 = inla(
  formula = Y ~ f(Region.struct, model = "besag", graph = g, hyper = list(prec=list(prior="loggamma", param=c(alpha_u,beta_u))), constr = FALSE) + 
    f(Region, model = "iid", hyper = list(prec=list(prior="loggamma", param=c(alpha_v,beta_v))), constr = FALSE)-1 + smoking, 
 family = "poisson",
 E=E,
 data = OralData
)
...
INLASmoking2 = inla(
  formula = Y ~ f(Region.struct, model = "besag", graph = g, hyper = list(prec=list(prior="loggamma", param=c(alpha_u,beta_u))), constr = FALSE) + 
    f(Region, model = "iid", hyper = list(prec=list(prior="loggamma", param=c(alpha_v,beta_v))), constr = FALSE) + f(smoking, model = 'rw2', hyper = list(prec = list(prior = "loggamma")), constr = FALSE)-1, 
 family = "poisson",
 E=E,
 data = OralData
)
