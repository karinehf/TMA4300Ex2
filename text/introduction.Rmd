...
The goal of this project is to study the distribution of oral cancer in Germany. We will mainly study this problem through the lens of a spatial Bayesian model, byt we will also briefly discuss the effects of smoking. Our data set is composed of the number of lip cancer patients in each district of Germany. We will assume that the number of cases are conditionally independent Poisson distrubuted variables. That is
$$
...
$$
Here $\textbf{R}$ is a neighbour matrix with $n_i$, the number of neighbour districts to district $i$, on the diagonal, $-1$ if district $i$ and $j$ are neighbours and zero otherwise. This will be a sparse non-negaitve definit matrix, and is therefore a possible precision matrix. The conditional distribution of $\boldsymbol{\eta}$ is then
$$
